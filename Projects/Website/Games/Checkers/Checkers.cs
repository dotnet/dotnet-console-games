...
using static Website.Games.Checkers.Statics;
...
public class Checkers
{
	...
	public async Task Run()
	{
		...
		try
		{
			...
			Game game = await ShowIntroScreenAndGetOption();
			await Console.Clear();
		...
		async Task<Game> ShowIntroScreenAndGetOption()
		{
			...
			await Console.WriteLine("  Checkers");
			await Console.WriteLine();
			await Console.WriteLine("  Checkers is played on an 8x8 board between two sides commonly known as black");
			await Console.WriteLine("  and white. The objective is simple - capture all your opponent's pieces. An");
			await Console.WriteLine("  alternative way to win is to trap your opponent so that they have no valid");
			await Console.WriteLine("  moves left.");
			await Console.WriteLine();
			await Console.WriteLine("  Black starts first and players take it in turns to move their pieces forward");
			await Console.WriteLine("  across the board diagonally. Should a piece reach the other side of the board");
			await Console.WriteLine("  the piece becomes a king and can then move diagonally backwards as well as");
			await Console.WriteLine("  forwards.");
			await Console.WriteLine();
			await Console.WriteLine("  Pieces are captured by jumping over them diagonally. More than one enemy piece");
			await Console.WriteLine("  can be captured in the same turn by the same piece. If you can capture a piece");
			await Console.WriteLine("  you must capture a piece.");
			await Console.WriteLine();
			await Console.WriteLine("  Moves are selected with the arrow keys. Use the [enter] button to select the");
			await Console.WriteLine("  from and to squares. Invalid moves are ignored.");
			await Console.WriteLine();
			await Console.WriteLine("  Press a number key to choose number of human players:");
			await Console.WriteLine("    [0] Black (computer) vs White (computer)");
			await Console.WriteLine("    [1] Black (human) vs White (computer)");
			await Console.Write("    [2] Black (human) vs White (human)");
		...
		async Task RunGameLoop(Game game)
		{
			while (game.Winner is null)
			{
				...
				await RenderGameState(game, playerMoved: currentPlayer, promptPressKey: true);
				await Console.ReadKey(true);
			}
		}

		async Task RenderGameState(Game game, Player? playerMoved = null, (int X, int Y)? selection = null, (int X, int Y)? from = null, bool promptPressKey = false)
		{
		...
		async Task<(int X, int Y)?> HumanMoveSelection(Game game, (int X, int y)? selectionStart = null, (int X, int Y)? from = null)
		{
			(int X, int Y) selection = selectionStart ?? (3, 3);
			while (true)
			{
				await RenderGameState(game, selection: selection, from: from);
				switch ((await Console.ReadKey(true)).Key)
				{
