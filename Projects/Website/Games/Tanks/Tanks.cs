...
using Console = Website.Console<Website.Games.Tanks.Tanks>;

namespace Website.Games.Tanks;
...
await Console.WriteLine("Use the (W, A, S, D) keys to move and the arrow keys to shoot.");
...
while (Tanks.Contains(Player) && Tanks.Count > 1)
{
	...
	foreach (var tank in Tanks)
	{
		...
		{
			tank.ExplodingFrame++;
			await Console.SetCursorPosition(tank.X - 2, tank.Y - 1);
			await Render(tank.ExplodingFrame > 9
				? TankExploding[2]
			...
			switch (direction)
			{
				case Direction.Up:
					if (MoveCheck(tank, tank.X, tank.Y - 1))
					{
						await Console.SetCursorPosition(tank.X - 2, tank.Y + 1);
						await Console.Write(' ');
						await Console.SetCursorPosition(tank.X - 1, tank.Y + 1);
						await Console.Write(' ');
						await Console.SetCursorPosition(tank.X, tank.Y + 1);
						await Console.Write(' ');
						await Console.SetCursorPosition(tank.X + 1, tank.Y + 1);
						await Console.Write(' ');
						await Console.SetCursorPosition(tank.X + 2, tank.Y + 1);
				...
				case Direction.Left:
					if (MoveCheck(tank, tank.X - 1, tank.Y))
					{
						await Console.SetCursorPosition(tank.X + 2, tank.Y - 1);
						await Console.Write(' ');
						await Console.SetCursorPosition(tank.X + 2, tank.Y);
						await Console.Write(' ');
						await Console.SetCursorPosition(tank.X + 2, tank.Y + 1);
						await Console.Write(' ');
						tank.X--;
		...
		else
		{
			...
			if (randomIndex < 4)
				await TryMove((Direction)randomIndex + 1);
...
await Console.Write(Tanks.Contains(Player)
	? "You Win."
	: "You Lose.");
