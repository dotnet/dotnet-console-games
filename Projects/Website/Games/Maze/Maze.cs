...
using Console = Website.Console<Website.Games.Maze.Maze>;
using OperatingSystem = Website.Console<Website.Games.Maze.Maze>;
...
public class Maze
{
	public static async Task Run()
	{
		...
		static MazeClass.Tile[,] GenerateMaze() =>
#if UsePrims
			MazeClass.GeneratePrims(rows, columns);
#else
			MazeClass.Generate(rows, columns);
#endif
#if MazeGenertorLoop
		while (true)
		{
			MazeClass.Tile[,] maze = GenerateMaze();
			Console.Clear();
		...
		MazeClass.Tile[,] maze = GenerateMaze();
		await Console.Clear();
		await Console.WriteLine(MazeClass.Render(maze));
		await Console.WriteLine();
		await Console.WriteLine("Maze");
		await Console.WriteLine("Solve the maze by using the arrow keys.");
		await Console.WriteLine("Press escape to quit.");
		int row = 0;
		int column = 0;
		while (row != rows - 1 || column != columns - 1)
		{
			await Console.SetCursorPosition(column * 3 + 1, row * 3 + 1);
			switch ((await Console.ReadKey(true)).Key)
			{
				case ConsoleKey.UpArrow:
					if (maze[row, column].HasFlag(MazeClass.Tile.Up))
						row--;
					break;
				case ConsoleKey.DownArrow:
					if (maze[row, column].HasFlag(MazeClass.Tile.Down))
						row++;
					break;
				case ConsoleKey.LeftArrow:
					if (maze[row, column].HasFlag(MazeClass.Tile.Left))
						column--;
					break;
				case ConsoleKey.RightArrow:
					if (maze[row, column].HasFlag(MazeClass.Tile.Right))
						column++;
					break;
...
#pragma warning restore IDE0042 // Deconstruct variable declaration
		...
		public class Graph
		{
			...
			public static MazeClass.Tile[,] ConvertToGrid(Graph graph, int rows, int columns, Func<int, int, int> index, int start_row, int start_column, int end_row, int end_column)
			{
				var tiles = new MazeClass.Tile[rows, columns];
				...
				foreach (var node in graph.Nodes)
				{
					...
					{
						tiles[row, col] |= MazeClass.Tile.Up;
						tiles[row - 1, col] |= MazeClass.Tile.Down;
					}
					if (node.Connections.Contains(index(row + 1, col)))
					{
						tiles[row, col] |= MazeClass.Tile.Down;
						tiles[row + 1, col] |= MazeClass.Tile.Up;
					}
					if (node.Connections.Contains(index(row, col - 1)))
					{
						tiles[row, col] |= MazeClass.Tile.Left;
						tiles[row, col - 1] |= MazeClass.Tile.Right;
					}
					if (node.Connections.Contains(index(row, col + 1)))
					{
						tiles[row, col] |= MazeClass.Tile.Right;
						tiles[row, col + 1] |= MazeClass.Tile.Left;
					}

					// start/end
					if (row == start_row && col == start_column)
					{
						tiles[row, col] |= MazeClass.Tile.Start;
					}
					if (row == end_row && col == end_column)
					{
						tiles[row, col] |= MazeClass.Tile.End;
					}
				}
				return tiles;
			}
